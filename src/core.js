...
export class UnkownAccountError extends CoreError {
  constructor(message) {
    super(message);
    this.name = "UnkownAccountError";
    this.code = "103";
  }
}
...
export class Ledger {
  ...
  getAccount(accountId) {
    ...
    if (!account) {
      throw new UnkownAccountError(`Account ${accountId} does not exist`);
    }
  ...
  printAccountTransactions(accountId) {
    ...
    this.transactions
      ...
      .forEach((t) =>
        console.log(
          `${id}\t\t${t.account}\t\t${t.amount}\t\t${t.status}\t\t${
            t.errorReason || "-"
          }\t\t${t.idempotencyToken || "-"}`
        )
      );
